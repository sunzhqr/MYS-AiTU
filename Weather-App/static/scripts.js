...
const mapboxToken =
  "pk.eyJ1IjoiczI3Mjg4IiwiYSI6ImNscnA4OW14cDAyZXAyam96Z25rZHdpd3QifQ.8r4lC0ndDr_xmmGzsWKjaQ";
...
if (localStorage.getItem(MODE_KEY)) {
  ...
  modeSlider.value =
    savedMode === "Light" ? "1" : savedMode === "Dark" ? "3" : "2";
...
function displayLocationSuggestions(suggestions) {
  ...
  suggestions.forEach((suggestion) => {
    ...
    listItem.onclick = () =>
      selectLocation(suggestion.coordinates, suggestion.place_name);
...
async function selectLocation(coordinates, place_name) {
  ...
  try {
    ...
    const mapBoxPrecipitation = document.getElementById(
      "map-box-precipitation"
    );
...
async function getForecastHourlyData(latitude, longitude) {
  const response = await fetch(
    `https://pro.openweathermap.org/data/2.5/forecast/hourly?lat=${latitude}&lon=${longitude}&appid=${apiKey}&units=metric&cnt=24`
  );
  const data = await response.json();
  return data.list;
}
...
async function getForecastDailyData(latitude, longitude) {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast/daily?lat=${latitude}&lon=${longitude}&appid=${apiKey}&units=metric&cnt=7`
  );
  const data = await response.json();
  return data.list;
}
...
async function getTA2MapData(latitude, longitude) {
  ...
  const TA2Map = L.map("map-box-temperature", {
    dragging: false,
  ...
  const weatherLayer = L.tileLayer(
    "http://maps.openweathermap.org/maps/2.0/weather/{op}/{z}/{x}/{y}?appid={apiKey}&opacity=0.6&fill_bound=true",
    {
      apiKey: apiKey,
      maxZoom: 18,
      attribution:
...
async function getPR0MapData(latitude, longitude) {
  ...
  const PR0Map = L.map("map-box-precipitation", {
    dragging: false,
    scrollWheelZoom: false,
    touchZoom: false,
    doubleClickZoom: false,
...
async function getWNDMapData(latitude, longitude) {
  ...
  const WNDMap = L.map("map-box-wind", {
    dragging: false,
    scrollWheelZoom: false,
    touchZoom: false,
    doubleClickZoom: false,
...
async function displayCurrentWeather(weather, place_name) {
  const currentWeatherTitle = document.getElementById("current-weather-title");
  var image = await getImage();
  console.log("image: ", image);
  currentWeatherTitle.innerHTML = `<h2>Current Weather in ${place_name}</h2> <img src="http://localhost:3000/image/${image}" style="z-index: 99999; width: 500px;">`;

  const currentWeatherElement = document.getElementById("current-weather");
  currentWeatherElement.innerHTML = `
    <p class="current-weather-description">Weather: ${capitalizeFirstLetter(
      weather.weather[0].description
    )}</p>
    <p class="current-weather-temperature">Temperature: ${Math.round(
      weather.main.temp
    )}°C</p>
    <p class="current-weather-feels-like">Feels Like: ${Math.round(
      weather.main.feels_like
    )}°C</p>
    <p class="current-weather-humidity">Humidity: ${Math.round(
      weather.main.humidity
    )}%</p>
    <p class="current-weather-pressure">Pressure: ${Math.round(
      weather.main.pressure
    )}hPa</p>
    <p class="current-weather-visibility">Visibility: ${
      Math.round((weather.visibility / 1000) * 10) / 10
    }km</p>
    <p class="current-weather-wind">Wind Speed: ${
...
function displayForecastHourlyData(forecastHourly, weather) {
  const forecastHourlyDataElement = document.getElementById(
    "forecast-hourly-box"
  );
...
function displayForecastDailyData(forecastDaily, weather) {
  const forecastDailyDataElement =
    document.getElementById("forecast-daily-box");
